...
This will spin up a Playwright-controlled browser which takes screenshots for various times of day and uses [wallpapper](https://github.com/mczachurski/wallpapper) to assemble them into an `output.heic` file in the `images/` directory. To set this gradient as your wallpaper on macOS, first change your wallpaper to the built-in "Solar Gradients" wallpaper. Then, right click on the output.heic file and choose "Set Desktop Picture".
